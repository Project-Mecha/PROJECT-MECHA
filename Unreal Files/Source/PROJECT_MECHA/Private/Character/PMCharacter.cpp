...
TArray<USkeletalMesh*> APMCharacter::GetBodyParts(FName PartName)
{
    if (PartsDataTable)
    {
        const FString ContextString = FName(PartName).ToString();
        PartData = PartsDataTable->FindRow<FMechPartsData>(PartName, ContextString, true);

        if (MechType == PartData->MechType)
        {
            return PartData->BodyParts;
        }
    }
    return TArray<USkeletalMesh*>();
}

TArray<UMaterialInterface*> APMCharacter::GetPartMaterials(FName PartName)
{
    if (PartsDataTable)
    {
        const FString ContextString = FName(PartName).ToString();
        ...
        if (MechType == PartData->MechType)
        {
            return PartData->PartMaterials;
        }
    }
    return TArray<UMaterialInterface*>();
}

FMeshInfo APMCharacter::CheckAndGetParts()
{
    if (UGameplayStatics::GetCurrentLevelName(this) == FString("RaceTrack"))
    {
        if (UPROJECTMECHA_Gameinstance* GameInstance = Cast<UPROJECTMECHA_Gameinstance>(UGameplayStatics::GetGameInstance(this)))
        {
            if (GameInstance->PlayerMeshInfo != FMeshInfo())
            {
                return GameInstance->PlayerMeshInfo;
            }
        }
    }
    return FMeshInfo();
