...
TYPE=""
DRY_RUN=""
BUILD_ROOT="C:/Users/Beyioku/Documents/Unreal Projects/Aptos-Cpp-SDK/build"
...
if [ ! -f "${BUILD_ROOT}/toolbox-0.1.0-Windows-AMD64.sh" ]; then
  echo "File ${BUILD_ROOT}/toolbox-0.1.0-Windows-AMD64.sh does not exists"
  exit 1
fi
...
if [ "${TYPE}" == "github" ]; then
  ...
  if [ "${DRY_RUN}" == "1" ]; then
    echo "${GHR_BIN} -replace 3.4.0 ${BUILD_ROOT}/toolbox-0.1.0-Windows-AMD64.sh"
    exit 0
  fi

  ${GHR_BIN} -replace 3.4.0 ${BUILD_ROOT}/toolbox-0.1.0-Windows-AMD64.sh
  ...
  # upload
  if [ "${DRY_RUN}" == "1" ]; then
    echo JFROG_CLI_OFFER_CONFIG=false ${JFROG_BIN} rt upload  "${BUILD_ROOT}/toolbox-0.1.0-Windows-AMD64.sh" ""
    exit 0
  fi

  JFROG_CLI_OFFER_CONFIG=false ${JFROG_BIN} rt upload  "${BUILD_ROOT}/toolbox-0.1.0-Windows-AMD64.sh" ""
